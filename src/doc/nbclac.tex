...
$$ R \;\; \mathcal{R}\tan\alpha  \;\; \mathrm{div} \vec{v} $$
...
\begin{equation}
	\Lambda_0 \sigma_{PFK}^\star = \Lambda^\star = \dfrac{S}{V}\left(\rho_{MCT_1} + \rho_{MCT_4}\right).
\end{equation}
We also need a parameter $\beta$ such that
\begin{equation}
	\left\lbrace
	\begin{array}{rcr}
	\dfrac{S}{V}\rho_{MCT_1} & = & \beta \Lambda^\star\\
	\\
	\dfrac{S}{V}\rho_{MCT_4} & = & (1-\beta) \Lambda^\star\\
	\end{array}
	\right.
\end{equation}
This parameter $\beta$ is the ratio of lactate elimination by $MCT_1$...

Assuming that the $MCT$ are not too sensitive to pH within the physiological ranges, we may model their fluxes by
\begin{equation}
	\left\lbrace
	\begin{array}{rcl}
	\rho_{MCT_1} & = & V_{m_1} \dfrac{\left[Lac^-\right]}{K_{m_1}+\left[Lac^-\right]}\\
	\\
	\rho_{MCT_4} & = & V_{m_4} \dfrac{\left[Lac^-\right]}{K_{m_4}+\left[Lac^-\right]}\\
	\end{array}
	\right.
\end{equation}
(we may remove the proton dependency, implicit in the lactate concentration).
Approximately, $MCT_1$ is able to expel the equivalent of $\sigma_{Lac}\approx 1mM/min$, so that, assuming the saturation
\begin{equation}
\sigma_{Lac} = \dfrac{S}{V} V_{m_1}
\end{equation}
Let us now assume that $L^\star$ is the steady state concentration of lactates.
\end{itemize}
